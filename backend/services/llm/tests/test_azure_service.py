"""Behavioral tests for Azure OpenAI generation service."""
...
def _make_azure_service(mock_client: AsyncMock) -> AzureService:
    """Create an AzureService with a pre-injected mock client."""
...
async def test_complete_returns_text_content_from_chat_response() -> None:
    ...
    mock_client.chat.completions.create = AsyncMock(
        return_value=_mock_completion_response("hello from azure")
    )
    service = _make_azure_service(mock_client)
...
@pytest.mark.asyncio
async def test_complete_wraps_azure_sdk_exception_in_llm_error() -> None:
    mock_client = AsyncMock()
    mock_client.chat.completions.create = AsyncMock(side_effect=Exception("sdk error"))
    service = _make_azure_service(mock_client)

    with pytest.raises(LLMError, match="Azure completion failed"):
